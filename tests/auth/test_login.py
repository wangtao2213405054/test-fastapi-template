...
async def test_edit_menu_info(client: AsyncClient, session: AsyncSession):
    ...
    add_result = await session.exec(select(MenuTable).where(MenuTable.id == _id))
    add_db_menu: MenuTable = add_result.first()
    ...
    update_result = await session.exec(select(MenuTable).where(MenuTable.id == _id))
    update_db_menu: MenuTable = update_result.first()
...
async def test_delete_menu_info(client: AsyncClient, session: AsyncSession):
    ...
    delete_result = await session.exec(select(MenuTable).where(MenuTable.id == db_menu.id))
